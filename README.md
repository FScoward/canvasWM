...
Place widgets on an infinite canvas, or manage real macOS windows spatially through a minimap overlay.
...
- **Canvas WM** (`Ctrl+T`) – Toggle Canvas WM mode
...
- Accessibility permissions (required for Canvas WM mode to move/resize windows)
...
├── CanvasWM/              # Canvas window manager
...
In Canvas WM mode, the viewport represents where the physical monitor sits on the canvas. Windows inside the viewport are shown on screen; windows outside are moved off-screen to (99999, 99999).
...
無限キャンバスにウィジェットを自由配置し、実際の macOS ウィンドウをミニマップで空間管理できます。

---

## 目次

- [Canvas WM モード](#canvas-wm-モード)
- [ウィジェット一覧](#ウィジェット一覧)
...
| ダブルクリック (空きエリア) | 付箋を追加 |
| スクロールホイール | ズーム |
| `Option + Control` (押し続ける) | Canvas WM ミニマップを一時表示 |
...
- **Canvas WM** (`Ctrl+T`) - Canvas WM モードのトグル
...
- macOS 14 (Sonoma) 以降
- アクセシビリティ権限 (Canvas WM モードでウィンドウの移動・リサイズに必要)
...
Canvas WM モードでは、ビューポートは物理モニターがキャンバス上のどこに位置するかを表します。ビューポート内のウィンドウは画面に表示され、ビューポート外のウィンドウはオフスクリーン (99999, 99999) に移動されます。
